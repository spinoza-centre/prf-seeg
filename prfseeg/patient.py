...
class Patient:
    ...
    def find_electrode_positions(self, which_run=0):
        ...
        """
        CT_orig = nb.load(os.path.join(self.raw_anat_dir, f'{self.subject}_CT.nii.gz'))
        T1w_orig = nb.load(os.path.join(self.raw_anat_dir, f'{self.subject}_T1w.nii.gz'))
        T1w_FS = nb.load(os.path.join(self.subjects_dir, self.subject, 'mri', 'T1.mgz'))

        if not os.path.isfile(T1w_FS):
            print('No FreeSurfer T1w file for this subject found. Please run FreeSurfer first. ')
            return

        reg_affine, _ = mne.transforms.compute_volume_registration(CT_orig, T1w_FS, pipeline='rigids')
        CT_aligned = mne.transforms.apply_volume_registration(CT_orig, T1w_FS, reg_affine)
        ...
        subj_trans = mne.coreg.estimate_head_mri_t(subject=self.subject, self.subjects_dir)
